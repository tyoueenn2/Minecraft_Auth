...
router.get('/status', requireAuth, (req, res) => {
  const records = db.getAccountsByDiscordId(req.session.user.id);
  res.json({
...
router.post('/link', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      if (existing.discord_id === req.session.user.id) {
...
router.post('/unlink', requireAuth, (req, res) => {
  const { uuid } = req.body;
  if (!uuid) {
    return res.status(400).json({ error: 'Minecraft UUID is required to unlink' });
  }

  const existing = db.getByMinecraftUuid(uuid);
  if (!existing || existing.discord_id !== req.session.user.id) {
    return res.status(404).json({ error: 'Account not found or not linked to you' });
  }

  db.unlinkByMinecraft(uuid);
