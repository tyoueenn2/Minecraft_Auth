...
module.exports = async function handleInteraction(interaction, client) {
  ...
  if (interaction.isButton() && interaction.customId === 'check-status-btn') {
    ...
    if (record) {
      const embed = new EmbedBuilder()
        ...
        .setTitle('✅ アカウント連携済み')
        .addFields(
          { name: '🎮 Minecraft', value: `\`${record.minecraft_username}\``, inline: true },
          { name: '📅 連携日時', value: `<t:${Math.floor(new Date(record.linked_at).getTime() / 1000)}:R>`, inline: true },
        )
        .setThumbnail(`https://mc-heads.net/avatar/${record.minecraft_uuid}/64`);
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'link-minecraft-modal') {
    ...
    try {
      ...
      if (existingMc && existingMc.discord_id !== interaction.user.id) {
        return interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xff4757)
