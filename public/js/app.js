...
const linkedAccountsList = $('#linked-accounts-list');
...
function renderLoggedIn(user, status) {
  ...
  if (!status.isInGuild) {
    ...
    linkedAccountsList.classList.add('hidden');
    return;
  }
  guildWarning.classList.add('hidden');
  
  // 常に連携フォームを表示（複数連携可能にするため）
  linkForm.classList.remove('hidden');

  if (status.linked && status.accounts && status.accounts.length > 0) {
    linkedAccountsList.classList.remove('hidden');
    statusUnlinked.classList.add('hidden');
    
    linkedAccountsList.innerHTML = '';
    status.accounts.forEach(account => {
      const accountDiv = document.createElement('div');
      accountDiv.className = 'status-linked'; // Reusing existing styling
      accountDiv.innerHTML = `
        <img class="mc-avatar" src="https://mc-heads.net/avatar/${account.minecraft_uuid}/56" alt="MCスキン">
        <div class="mc-info" style="flex: 1;">
          <h3>${account.minecraft_username}</h3>
          <p>${account.minecraft_uuid}</p>
        </div>
        <button class="btn btn-red unlink-specific-btn" data-uuid="${account.minecraft_uuid}" style="padding: 8px 16px; font-size: 13px;">解除</button>
      `;
      linkedAccountsList.appendChild(accountDiv);
    });

    document.querySelectorAll('.unlink-specific-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        const uuid = e.target.getAttribute('data-uuid');
        if (!confirm('このMinecraftアカウントの連携を解除しますか？')) return;
        e.target.disabled = true;
        try {
          const result = await api('/api/unlink', { method: 'POST', body: JSON.stringify({ uuid }) });
          if (result.error) {
